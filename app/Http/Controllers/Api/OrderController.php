        ...
        if (is_array($request->all())) {
            collect($request->all())->map(function($data){
                ...
                if ($product->stock >= $std->order_detail->amount){
                    ...
                    $order_detail = OrderDetail::create([
                        ...
                        'total_order' => $std->order_detail->total_order,
                        'cost' => $std->order_detail->cost
                        ]);
                }
            });
        } else {
            ...
            if ($product->stock >= $request->input('order_detail.amount')){
                ...
                $order_detail = OrderDetail::create([
                    ...
                    'total_order' => $request->input('order_detail.total_order'),
                    'cost' => $request->input('order_detail.cost')
                    ]);
            }
        }
